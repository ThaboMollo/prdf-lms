...
**Manual verification still needed against the real project** (not done by the implementing agent — no access): the actual Supavisor pooler connection (only the transaction-local `set_config` mechanism was verified, against a local Postgres standing in for it), and a real, successful Vercel deployment of `backend-node` with the current code (a `prdf-api` Vercel project exists and is git-connected to this repo, but as of this writing had only ever deployed the pre-Phase-3a commit — the Phase 3a changes were still sitting uncommitted locally).
