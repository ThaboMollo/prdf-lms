...
- **Term units:** client model uses days; `loan_products` uses months — reconcile in Phase 2 product redesign. Interim: the portal converts at `daysPerYear / 12`, which hits the sheet's 365/730/1095-day rows exactly but reads its 90d and 180d rows as 91 and 183 (the sheet treats those two as 30-day months). Pinned by `client-ui/scripts/test-credit-quote.mjs`.
- **Products decision (PARTLY RESOLVED 2026-09-25):** for now neither — "PRDF Standard" simply had its band narrowed to the union of the two, R250,000–R1,000,000 (`20260925120000_client_portal_loan_band.sql`). The portal therefore cannot express that ≤R500k is 90-day PO funding while >R500k runs 180–1095 days; a R300k / 36-month quote is reachable in the UI though the sheet never models it. Splitting into two real products is still Phase 2 schema work.
- **Client-portal quote visibility (RESOLVED 2026-09-25):** applicants now see an indicative quote. The public `LoanCalculator` and the apply wizard's cost card both price through this engine via `client-ui/src/lib/creditQuote.ts`, fed by an unauthenticated `GET /api/pricing/public-config`. That endpoint returns the fee structure plus **only the cheapest active margin** — never the grade ladder — so the portal quotes "From 15.50% p.a." without publishing what a Worst-graded applicant pays. `POST /api/pricing/quote` stays staff-only and `ensureInternal`-gated.
