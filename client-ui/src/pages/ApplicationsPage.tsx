...
export function ApplicationsPage({ session, me }: ApplicationsPageProps) {
  ...
  const abandonMutation = useMutation({
    mutationFn: (id: string) => applicationsUseCases.transitionStatus(id, 'Withdrawn', 'Client withdrew application'),
    onSuccess: async () => {
      toast.push('Application withdrawn.', 'success')
      await refreshSelected(queryClient, selectedApplicationId)
    },
    onError: () => toast.push('Could not withdraw application.', 'error')
  })
  ...
  const clientDrafts = allApplications.filter((app) => app.status === 'Draft' || app.status === 'InfoRequested')
  const clientActiveId = selectedApplicationId && clientDrafts.some((app) => app.id === selectedApplicationId)
    ? selectedApplicationId
    : clientDrafts[0]?.id ?? selectedApplicationId ?? null

  useEffect(() => {
    if (!isInternal && clientActiveId && clientActiveId !== selectedApplicationId) {
      const next = new URLSearchParams(params)
      next.set('app', clientActiveId)
      setParams(next, { replace: true })
    }
  }, [clientActiveId, isInternal, params, selectedApplicationId, setParams])

  if (!isInternal) {
    const docs = docsQuery.data ?? []
    const activeApplication = detail
    const missingDocs = requiredDocumentTypes.filter((requiredDoc) => !docs.some((doc) => doc.docType === requiredDoc))
    const canSubmit = activeApplication && missingDocs.length === 0 && activeApplication.status === 'Draft'
    const canWithdraw = activeApplication && (activeApplication.status === 'Draft' || activeApplication.status === 'InfoRequested')

    return (
      <section className="stack client-apply">
        <PageHeader
          title="Complete your application"
          subtitle="Upload the required documents before submitting your application."
          actions={(
        ...
        {applicationsQuery.isLoading ? <DetailSkeleton /> : null}

        {!applicationsQuery.isLoading && !allApplications.length ? (
          <EmptyState title="No applications yet" message="Start a new application to begin the process." />
        ) : null}

        <article className="card apply-card">
          <div className="apply-header">
            <div>
              <h2>Document upload</h2>
              <p className="muted-text">Upload each required document. You can submit once everything is attached.</p>
            </div>
            {clientDrafts.length ? (
              <label className="apply-select">
                Draft application
                <select
                  value={clientActiveId ?? ''}
                  onChange={(event) => {
                    const next = new URLSearchParams(params)
                    next.set('app', event.target.value)
                    setParams(next)
                  }}
                >
                  {clientDrafts.map((app) => (
                    <option key={app.id} value={app.id}>
                      {app.id.slice(0, 8)} · {formatCurrency(app.requestedAmount)}
                    </option>
                  ))}
                </select>
              </label>
          ...
          {!clientDrafts.length ? (
            <EmptyState title="No draft application" message="Create a draft so you can upload documents and submit." />
          ) : null}
          ...
          {clientDrafts.length ? (
            <div className="apply-grid">
              ...
              <div className="stack-sm">
                ...
                <div
                  className={`dropzone${docFile ? ' dropzone-active' : ''}`}
                ...
                >
                  ...
                  <button
                    ...
                    disabled={uploadMutation.isPending || !docFile || !clientActiveId}
                  >
                ...
                <div className="inline-actions">
                  <button
                    className="btn"
                    type="button"
                    onClick={() => clientActiveId ? submitMutation.mutate(clientActiveId) : null}
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() => clientActiveId ? abandonMutation.mutate(clientActiveId) : null}
                    disabled={!canWithdraw || abandonMutation.isPending}
                  >
                    {abandonMutation.isPending ? 'Withdrawing...' : 'Abandon Application'}
                  </button>
                </div>
              </div>
            </div>
