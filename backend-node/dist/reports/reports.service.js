...
let ReportsService = class ReportsService {
    ...
    async pipelineSummary(actor, startDate, endDate) {
        await this.ensureStaff(actor);
        return this.db.query(`select status, cast(count(*) as int) as count, cast(coalesce(sum(requested_amount), 0) as double precision) as "totalAmount"
       from public.loan_applications
       where ($1::timestamptz is null or created_at >= $1::timestamptz)
         and ($2::timestamptz is null or created_at <= $2::timestamptz)
       group by status
       order by count(*) desc`, [startDate ?? null, endDate ?? null]);
    }
    async originationTrends(actor, startDate, endDate) {
        await this.ensureStaff(actor);
        return this.db.query(`select to_char(disbursed_at, 'YYYY-MM') as month,
       ...
       group by 1
       order by 1 asc`, [startDate ?? null, endDate ?? null]);
    }
};
