...
begin
  ...
  filtered as (
    select
      ...
      coalesce(ra.is_client, false) as is_client
    from role_agg ra
    where (
        p_search is null
  ...
  select
    ...
    case
      ...
      when not f.is_internal then 'Only internal users are eligible for Admin access.'
      else null
...
begin
  ...
  if p_role_name not in ('Client', 'Intern', 'Originator', 'LoanOfficer') then
