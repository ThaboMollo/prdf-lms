 ...
 * (previously direct Supabase reads). Mirrors client-ui/src/lib/loanProduct.ts
 * — duplicated rather than shared, since there's no shared package yet
 * (that's Phase 4).
 */
...
/** Deliberately unauthenticated — backend-node's GET /api/loan-products/active is a public route (mirrors the DB's anon-readable RLS scope for this one row). */
